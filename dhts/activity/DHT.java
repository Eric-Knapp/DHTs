...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	/*
	 * This version gets the local predecessor from routing tables.
	 */
	// WebMethod
...
//		routing.joinCheck();
	...
	public TableRep notify(TableRep predDb) {
		...
		if (pred == null || inInterval(cand.id, pred.id, info.id, false)) {
			...
			Log.debug(TAG, "Transferring bindings to node id=" + cand.id);
			return db;
		} else {
			/*
	...
	public void join(String uri) throws Failed, Invalid {
		...
		UriBuilder UserInterfaceBuilder = UriBuilder.fromPath(uri); 
		
		URI ub = UserInterfaceBuilder.build(); 
		succ = findSuccessor(ub,info.id); 
		setSucc(succ); 
		state.clear(); 
		stabilize(); 
	}
